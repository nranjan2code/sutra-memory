...
use sutra_storage::{
    ...
    AdaptiveReconcilerConfig, ConceptId, ConceptNode, ConcurrentConfig, ConcurrentMemory,
    ConcurrentStats, ParallelPathFinder,
};
...
// Core UI imports
use crate::ui::{
    ChatAction, ChatPanel, ConnectionStatus, KnowledgeAction, KnowledgePanel, QuickLearnAction,
    QuickLearnPanel, SettingsAction, SettingsPanel, Sidebar, SidebarView, StatusBar,
    StorageStatsUI, StorageStatus,
};

// Enhanced UI imports
use crate::ui::{
    AnalyticsAction, AnalyticsDashboard, CausalView, CausalViewAction, ExportImportAction,
    ExportImportPanel, GraphAction, GraphView, QueryBuilder, QueryBuilderAction,
    ReasoningPathsAction, ReasoningPathsPanel, TemporalView, TemporalViewAction,
};

// New UX features (v3.4)
use crate::ui::{
    is_first_launch, mark_onboarding_complete, CommandHistory,
    HomeAction, HomeDashboard, OnboardingAction, OnboardingTour,
};

use crate::theme::{apply_theme, BG_DARK, BG_PANEL};
use crate::types::{
    AppMessage, ConceptInfo,
    EdgeType, GraphEdge, GraphNode, ReasoningPath,
};
...
impl SutraApp {
        ...
        match action {
            ...
            ChatAction::Query(query) => {
                ...
                std::thread::spawn(move || {
                    ...
                    let result_data: Vec<(ConceptId, String, f32)> = results
                        .into_iter()
                        .collect();
                    ...
                    let generated_response = if let Some(nlg_provider) = nlg {
                        if !result_data.is_empty() {
                            ...
                            let context: String = result_data
                                .iter()
                                .take(3)
            ...
            ChatAction::Clear => {
                ...
                self.chat
                    .add_response("🧹 Chat cleared. Ready for new conversations!".to_string());
    ...
    fn handle_knowledge_action(&mut self, action: KnowledgeAction) {
        match action {
            ...
            KnowledgeAction::DeleteConcept(id_str) => {
                ...
                std::thread::spawn(move || match storage.delete_concept(concept_id) {
                    Ok(_) => {
    ...
    fn handle_quick_learn_action(&mut self, action: QuickLearnAction) {
        match action {
            ...
            QuickLearnAction::BatchLearn(batch_content) => {
                ...
                let batch_lines: Vec<String> = batch_content
                    .lines()
                ...
                for line in &batch_lines {
                    self.quick_learn
                        .add_learn_entry(line.clone(), LearnStatus::Processing);
                }
    ...
    fn handle_graph_action(&mut self, action: GraphAction) {
        match action {
                    ...
                    for concept in &concepts {
                        for neighbor_id in &concept.neighbors {
                            ...
                            if concept.id.to_hex() < neighbor_id.to_hex() {
                                let content =
                                    String::from_utf8_lossy(&concept.content).to_lowercase();
                                let edge_type = if content.contains("causes")
                                    || content.contains("leads to")
                                    || content.contains("results in")
                                {
                                    EdgeType::Causal
                                } else if content.contains("before")
                                    || content.contains("after")
                                    || content.contains("during")
                                {
                                    EdgeType::Temporal
                                } else if content.contains("is a")
                                    || content.contains("part of")
                                    || content.contains("contains")
                                {
                                    EdgeType::Hierarchical
                                } else if content.contains("similar")
            ...
            GraphAction::SelectNode(id) => {
                ...
                self.status_bar
                    .set_activity(format!("Selected node: {}...", &id.to_hex()[..8]));
            }
            GraphAction::ExportImage => {
                ...
                if let Some(home) = dirs::home_dir() {
                    ...
                    match std::fs::write(&path, graphml) {
                        Ok(_) => {
                            self.status_bar
                                .set_activity(format!("Graph exported to {:?}", path));
                        }
                        Err(e) => {
                            self.status_bar
                                .set_activity(format!("Export failed: {}", e));
                        }
                    }
                } else {
                    self.status_bar
                        .set_activity("Could not determine home directory");
                }
            }
        }
    }
    ...
    fn handle_reasoning_action(&mut self, action: ReasoningPathsAction) {
        match action {
                        ...
                        (from_results.first(), to_results.first())
                    {
                        ...
                        if raw_paths.is_empty() {
                            ...
                            let paths: Vec<ReasoningPath> = raw_paths
                                .into_iter()
                                .map(|p| {
                                    ReasoningPathsPanel::convert_path_static(&p, &snapshot, decay)
                                })
                                .collect();

                            // Analyze consensus
                            let consensus =
                                ReasoningPathsPanel::analyze_consensus_static(&paths, threshold);
            ...
            ReasoningPathsAction::ExportReasoning => {
                ...
                let export_data: Vec<serde_json::Value> = paths
                    .iter()
                    .enumerate()
                    .map(|(idx, path)| {
                ...
                if let Some(home) = dirs::home_dir() {
                    ...
                    match std::fs::write(&path, json_str) {
                        Ok(_) => {
                            self.status_bar
                                .set_activity(format!("Reasoning paths exported to {:?}", path));
                        }
                        Err(e) => {
                            self.status_bar
                                .set_activity(format!("Export failed: {}", e));
    ...
    fn handle_temporal_action(&mut self, action: TemporalViewAction) {
        match action {
            ...
            TemporalViewAction::ExploreRelations(_id, relation) => {
                // Search for related concepts with temporal relationships
                let results = self
                    .storage
                    .text_search(&format!("{:?}", relation).to_lowercase(), 10);
                if !results.is_empty() {
                ...
                self.status_bar
                    .set_activity(format!("Found {} related concepts", results.len()));
            }
            TemporalViewAction::RefreshData => {
                ...
                self.temporal_view.load_events(events);
                self.status_bar
                    .set_activity(format!("Loaded {} temporal events", event_count));
            }
        }
    }
    ...
    fn handle_causal_action(&mut self, action: CausalViewAction) {
        match action {
            CausalViewAction::AnalyzeCause { effect, max_hops } => {
                ...
                self.status_bar
                    .set_activity(format!("Analyzing causes for '{}'...", effect));
                ...
                std::thread::spawn(move || {
                    ...
                    if let Some((effect_id, _, _)) = results.first() {
                        let (chains, root_causes) =
                            CausalView::analyze_static(&storage, *effect_id, max_hops);
            ...
            CausalViewAction::ExportChains => {
                ...
                let chains_export: Vec<serde_json::Value> = chains
                    .iter()
                    .enumerate()
                    .map(|(idx, chain)| {
                ...
                let root_causes_export: Vec<serde_json::Value> = root_causes
                    .iter()
                    .map(|cause| {
                        serde_json::json!({
                            "id": cause.id,
                ...
                if let Some(home) = dirs::home_dir() {
                    ...
                    match std::fs::write(&path, json_str) {
                        Ok(_) => {
                            self.status_bar
                                .set_activity(format!("Causal analysis exported to {:?}", path));
                        }
                        Err(e) => {
                            self.status_bar
                                .set_activity(format!("Export failed: {}", e));
            ...
            AnalyticsAction::ExportReport => {
                ...
                if let Some(home) = dirs::home_dir() {
                    ...
                    match std::fs::write(&path, json_str) {
                        Ok(_) => {
                            self.status_bar
                                .set_activity(format!("Analytics report exported to {:?}", path));
                        }
                        Err(e) => {
                            self.status_bar
                                .set_activity(format!("Export failed: {}", e));
            ...
            QueryBuilderAction::RunQuery {
                ...
                std::thread::spawn(move || {
                    ...
                    let results: Vec<ConceptInfo> = {
                        let search_results =
                            storage.text_search(&query_clone, filters_clone.max_results);
                        search_results
                            .into_iter()
            ...
            QueryBuilderAction::ExportResults => {
                ...
                if let Some(home) = dirs::home_dir() {
                    ...
                    match std::fs::write(&path, json_str) {
                        Ok(_) => {
                            self.status_bar
                                .set_activity(format!("Query results exported to {:?}", path));
                        }
                        Err(e) => {
                            self.status_bar
                                .set_activity(format!("Export failed: {}", e));
    ...
    fn handle_export_import_action(&mut self, action: ExportImportAction) {
        match action {
            ExportImportAction::Export(path) => {
                self.status_bar
                    .set_activity(format!("Exporting to {}...", path));
            ...
            ExportImportAction::Import(path) => {
                self.status_bar
                    .set_activity(format!("Importing from {}...", path));
                ...
                match std::fs::read_to_string(&expanded_path) {
                    Ok(content) => match self.import_json_data(&content) {
                        Ok((concepts, edges)) => {
                            self.export_import.set_import_result(
                                ...
                                0,
                                &format!(
                                    "Successfully imported {} concepts, {} edges",
                                    concepts, edges
                                ),
                            );
                            self.refresh_stats();
                            self.status_bar
                                .set_activity(format!("Imported {} concepts", concepts));
            ...
            ExportImportAction::BatchImport(path) => {
                self.status_bar
                    .set_activity(format!("Batch importing from {}...", path));
            ...
            ExportImportAction::CancelBatch => {
                ...
                self.status_bar
                    .set_activity("Batch operation cancelled (background task may continue)");
            }
        }
    }
    ...
    fn handle_settings_action(&mut self, action: SettingsAction, ctx: &egui::Context) {
        match action {
            SettingsAction::Save => match self.storage.flush() {
                Ok(_) => {
            ...
            SettingsAction::ClearData => {
                ...
                self.chat.messages.clear();
                self.chat
                    .add_response("🗑️ All data has been cleared. Starting fresh!".to_string());
        ...
        match action {
            OnboardingAction::Next => {
                ...
                if !self.onboarding.should_show() {
                    ...
                    self.status_bar
                        .set_activity("Tour completed! Welcome to Sutra AI.");
                }
            }
            ...
            OnboardingAction::Dismiss => {
                ...
                mark_onboarding_complete();
                self.status_bar
                    .set_activity("Tour skipped. You can restart it from Settings.");
            }
            ...
            match msg {
                AppMessage::LearnResult {
                    ...
                    if let Some(err) = error {
                        self.chat
                            .add_response(format!("❌ Failed to learn: {}", err));
                        ...
                        self.status_bar
                            .set_activity(format!("Learned: {}... ({}ms)", preview, duration_ms));
                        self.refresh_stats();
                    }
                }
                ...
                AppMessage::SearchResults {
                    ...
                    } else if let Some(response) = generated_response {
                    ...
                    let preview = if query.len() > 30 {
                        &query[..30]
                    } else {
                        &query
                    };
                    self.status_bar
                        .set_activity(format!("Searched: {} ({}ms)", preview, duration_ms));
                }
                ...
                AppMessage::QuickLearnCompleted {
                    content,
                    success,
                    error,
                    elapsed_ms,
                    ...
                    if let Some(entry) = self.quick_learn.recent_learns.iter_mut().find(|e| {
                        e.content == content
                            && matches!(e.status, crate::ui::quick_learn::LearnStatus::Processing)
                    }) {
                        entry.status = if success {
                            crate::ui::quick_learn::LearnStatus::Success
                        } else {
                            crate::ui::quick_learn::LearnStatus::Failed(
                                error.clone().unwrap_or_else(|| "Unknown error".to_string()),
                            )
                        };

                        // Check if this was the last processing item
                        let still_processing = self.quick_learn.recent_learns.iter().any(|e| {
                            matches!(e.status, crate::ui::quick_learn::LearnStatus::Processing)
                        });
                ...
                AppMessage::BatchLearnCompleted {
                    total,
                    successes,
                    failures,
                    elapsed_ms,
                    ...
                    let status_msg = if failures.is_empty() {
                        format!(
                            "Batch complete: {}/{} learned ({}ms)",
                            successes, total, elapsed_ms
                        )
                    } else {
                        format!(
                            "Batch partial: {}/{} learned, {} failed ({}ms)",
                            successes,
                            total,
                            failures.len(),
                            elapsed_ms
                ...
                AppMessage::ReasoningPathsFound {
                    paths,
                    consensus,
                    error,
                } => {
                    ...
                    if let Some(err) = error {
                        ...
                        self.status_bar.set_activity(format!(
                            "Found {} paths",
                            self.reasoning_paths.paths.len()
                        ));
                    }
                }

                AppMessage::CausalAnalysisComplete {
                    chains,
                    root_causes,
                    error,
                } => {
                ...
                AppMessage::QueryBuilderResults {
                    results,
                    duration_ms,
                } => {
                    ...
                    self.status_bar
                        .set_activity(format!("Query completed in {}ms", duration_ms));
                }

                AppMessage::BatchImportProgress {
                    completed,
                    total,
                    errors,
                } => {
                ...
                AppMessage::BatchImportComplete {
                    completed,
                    errors,
                    error,
                } => {
                    ...
                    if let Some(err) = error {
                        self.status_bar
                            .set_activity(format!("Batch import failed: {}", err));
                ...
                AppMessage::ConceptDeleted {
                    ...
                    if success {
                        ...
                        self.status_bar
                            .set_activity(format!("Delete failed: {}", err));
    ...
    fn render_menu_bar(&mut self, ctx: &egui::Context) {
        use crate::theme::{BG_PANEL, TEXT_PRIMARY, TEXT_SECONDARY};
    ...
    fn refresh_stats(&mut self) {
        let stats = self.storage.stats();
        self.status_bar
            .set_concept_count(stats.snapshot.concept_count);
    ...
    fn import_json_data(&self, content: &str) -> Result<(usize, usize), String> {
        ...
        for concept in concepts {
            ...
            let confidence = concept
                .get("confidence")
                .and_then(|c| c.as_f64())
                .unwrap_or(1.0) as f32;

            let strength = concept
                .get("strength")
                .and_then(|s| s.as_f64())
                .unwrap_or(1.0) as f32;
    ...
    pub fn load_temporal_events(&self) -> Vec<crate::types::TimelineEvent> {
        ...
        let temporal_keywords = [
            "before", "after", "during", "when", "then", "until", "since", "while",
        ];
        ...
        concepts
            ...
            .filter_map(|node| {
                ...
                let is_semantic_temporal = node
                    .semantic
                    .as_ref()
                    .map(|s| {
                        s.semantic_type == SemanticType::Temporal
                            || s.semantic_type == SemanticType::Event
                    })
                    .unwrap_or(false);
                ...
                if has_temporal || is_semantic_temporal {
                    ...
                    let relative_time =
                        if content_lower.contains("before") || content_lower.contains("earlier") {
                            -1
                        } else if content_lower.contains("after")
                            || content_lower.contains("later")
            ...
            for file in dropped_files {
                if let Some(path) = file.path {
                    ...
                    if path_str.to_lowercase().ends_with(".json") {
                        ...
                        self.export_import.active_tab =
                            crate::ui::export_import::ExportImportTab::Import;
                        ...
                        self.export_import.active_tab =
                            crate::ui::export_import::ExportImportTab::Batch;
                        self.handle_export_import_action(ExportImportAction::BatchImport(path_str));
                    } else {
                        self.status_bar
                            .set_activity(format!("Unsupported file type: {}", path_str));
