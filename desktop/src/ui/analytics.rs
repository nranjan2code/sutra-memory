...
use crate::theme::*;
use crate::types::{AnalyticsMetrics, MetricsSnapshot, ActivityEntry, ActivityType, QueryLogEntry};
...
impl AnalyticsDashboard {
    ...
    pub fn update(&mut self, storage: &ConcurrentMemory) {
        ...
        if stats.snapshot.concept_count > 0 {
            self.metrics.hnsw_coverage = hnsw.indexed_vectors as f32 / stats.snapshot.concept_count as f32;
        }
        ...
        if !self.latency_samples.is_empty() {
            ...
            self.metrics.p95_query_latency_ms = sorted.get((sorted.len() as f32 * 0.95) as usize).copied().unwrap_or(0.0);
            self.metrics.p99_query_latency_ms = sorted.get((sorted.len() as f32 * 0.99) as usize).copied().unwrap_or(0.0);
        }
    ...
    pub fn record_query(&mut self, query: &str, duration: Duration, results: usize) {
        ...
        // Add to activity log
        self.log_activity(ActivityType::Query, format!("{}", query), Some(duration.as_millis() as u64));
    }
    ...
    /// Log an activity
    pub fn log_activity(&mut self, activity_type: ActivityType, description: String, duration_ms: Option<u64>) {
    ...
    fn render_header(&mut self, ui: &mut egui::Ui, action: &mut Option<AnalyticsAction>) {
        ui.horizontal(|ui| {
            ...
            ui.add_space(4.0);
            ui.label(RichText::new("Analytics Dashboard").size(22.0).color(TEXT_PRIMARY).strong());
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                ...
                egui::ComboBox::from_id_salt("time_range")
                    ...
                    .show_ui(ui, |ui| {
                        ...
                        ui.selectable_value(&mut self.time_range, TimeRange::Last24Hours, "Last 24h");
                ...
                if ui.button(RichText::new("📤 Export Report").size(12.0)).clicked() {
    ...
    fn render_knowledge_growth(&self, ui: &mut egui::Ui) {
        self.stat_card(ui, "Knowledge Growth", |ui| {
            ui.horizontal(|ui| {
                ui.label(RichText::new(format!("{}", self.metrics.total_concepts))
                    .size(36.0).color(PRIMARY).strong());
                
                // Growth indicator
                if self.metrics.concepts_today > 0 {
                    ui.label(RichText::new(format!("↗ +{}", self.metrics.concepts_today))
                        .size(14.0).color(SUCCESS));
                }
            });
            ...
            let chart_rect = ui.allocate_exact_size(Vec2::new(ui.available_width() - 20.0, 40.0), egui::Sense::hover()).0;
    ...
    fn render_learning_activity(&self, ui: &mut egui::Ui) {
        self.stat_card(ui, "Learning Activity", |ui| {
            ui.horizontal(|ui| {
                ui.label(RichText::new(format!("+{}", self.metrics.concepts_today))
                    .size(36.0).color(SUCCESS).strong());
            });
            ...
            ui.horizontal(|ui| {
                egui::Frame::none()
                    ...
                    .show(ui, |ui| {
                        ui.label(RichText::new(format!("This hour: +{}", self.metrics.concepts_this_hour))
        ...
        self.stat_card(ui, "Query Performance", |ui| {
            ui.horizontal(|ui| {
                ui.vertical(|ui| {
                    ...
                    ui.label(RichText::new(format!("{:.0}ms", self.metrics.avg_query_latency_ms))
                        .size(24.0).color(SECONDARY).strong());
                });
                ...
                ui.vertical(|ui| {
                    ...
                    ui.label(RichText::new(format!("{:.0}ms", self.metrics.p95_query_latency_ms))
                        .size(24.0).color(ACCENT).strong());
                });
                ...
                ui.vertical(|ui| {
                    ...
                    ui.label(RichText::new(format!("{:.0}ms", self.metrics.p99_query_latency_ms))
                        .size(24.0).color(WARNING).strong());
                });
            });
            ...
            // Latency chart
            let chart_rect = ui.allocate_exact_size(Vec2::new(ui.available_width() - 20.0, 30.0), egui::Sense::hover()).0;
    ...
    fn render_storage_stats(&self, ui: &mut egui::Ui) {
        self.stat_card(ui, "Storage Statistics", |ui| {
            let stats = [
                ("Concepts", format!("{}", self.metrics.total_concepts), PRIMARY),
        ...
        self.stat_card(ui, "Top Queries Today", |ui| {
            if self.top_queries.is_empty() {
                ...
                for (i, (query, count)) in self.top_queries.iter().take(5).enumerate() {
                    ui.horizontal(|ui| {
                        ui.label(RichText::new(format!("{}.", i + 1)).size(11.0).color(TEXT_MUTED));
                        ...
                        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                            ui.label(RichText::new(format!("({})", count)).size(11.0).color(SECONDARY));
                        });
                    });
                }
            }
    ...
    fn render_hnsw_health(&self, ui: &mut egui::Ui) {
        self.stat_card(ui, "HNSW Index Health", |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    ui.label(RichText::new(format!("{:.0}%", self.metrics.hnsw_coverage * 100.0))
                        .size(14.0).color(SUCCESS).strong());
                });
            });
            ...
            // Progress bar
            let bar_rect = ui.allocate_exact_size(Vec2::new(ui.available_width() - 20.0, 8.0), egui::Sense::hover()).0;
            ui.painter().rect_filled(bar_rect, Rounding::same(4.0), BG_DARK);
            ...
            ui.painter().rect_filled(fill_rect, Rounding::same(4.0), SUCCESS);
            
            ui.add_space(8.0);
            
            ui.horizontal(|ui| {
                ui.label(RichText::new(format!("Indexed: {}/{}", 
            ...
            egui::ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    if self.activity_log.is_empty() {
                        ui.label(RichText::new("No recent activity").size(12.0).color(TEXT_MUTED));
                    } else {
    ...
    fn render_activity_entry(&self, ui: &mut egui::Ui, entry: &ActivityEntry) {
        ui.horizontal(|ui| {
            // Timestamp
            ui.label(RichText::new(entry.timestamp.format("%H:%M").to_string())
                .size(10.0).color(TEXT_MUTED).monospace());
            
            ui.add_space(8.0);
            
            // Activity type icon and color
            let (icon, color) = match entry.activity_type {
            ...
            if let Some(ms) = entry.duration_ms {
                ui.label(RichText::new(format!("({}ms)", ms)).size(10.0).color(TEXT_MUTED));
            }
        });
    }
    ...
    fn draw_sparkline(&self, ui: &mut egui::Ui, rect: Rect, color: Color32) {
        ...
        let points: Vec<_> = self.history
            ...
            .map(|(i, h)| {
                ...
                let max_val = self.history.iter().map(|h| h.concept_count).max().unwrap_or(1) as f32;
        ...
        if points.len() > 1 {
            for window in points.windows(2) {
                ui.painter().line_segment([window[0], window[1]], Stroke::new(2.0, color.gamma_multiply(0.7)));
            }
        }
    }
    ...
    fn draw_latency_bars(&self, ui: &mut egui::Ui, rect: Rect) {
        ...
        for (i, &latency) in self.latency_samples.iter().rev().take(50).enumerate() {
            ...
            ui.painter().rect_filled(bar, Rounding::same(1.0), color.gamma_multiply(0.7));
        }
    }
}
