...
use eframe::egui::{self, Color32, Pos2, RichText, Rounding, ScrollArea, Stroke, Vec2};
...
impl CausalView {
            ...
            .show(ui, |ui| {
                ui.horizontal(|ui| {
                    ...
                    ui.vertical(|ui| {
                        ui.add_space(18.0);
                        ui.checkbox(
                            &mut self.include_indirect,
                            RichText::new("Include indirect")
                                .size(11.0)
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        ...
                        if ui
                            .add_enabled(
                                can_analyze,
                                egui::Button::new(
                                    RichText::new("🔍 Analyze").size(13.0).color(Color32::WHITE),
                                )
                                .fill(ACCENT)
    ...
    fn render_analysis_view(&mut self, ui: &mut egui::Ui, _action: &mut Option<CausalViewAction>) {
        // Error message
        ...
        if self.is_analyzing {
            ui.vertical_centered(|ui| {
                ...
                ui.add_space(8.0);
                ui.label(
                    RichText::new("Analyzing causal relationships...")
                        .size(13.0)
                        .color(TEXT_MUTED),
                );
            });
        ...
        if self.causal_chains.is_empty() {
            ui.vertical_centered(|ui| {
                ...
                ui.add_space(16.0);
                ui.label(
                    RichText::new("Root Cause Analysis")
                        .size(18.0)
                        .color(TEXT_PRIMARY)
                        .strong(),
                );
                ui.add_space(8.0);
                ui.label(
                    RichText::new("Trace effects back to their origins across the graph.")
                        .size(14.0)
                        .color(TEXT_MUTED),
                );
                ui.add_space(24.0);
                ...
                egui::Frame::none()
                    ...
                    .show(ui, |ui| {
                        ...
                        ui.add_space(8.0);
                        ui.label(
                            RichText::new("Or teach causal links:")
                                .size(12.0)
                                .color(TEXT_SECONDARY),
                        );
                        ui.add_space(4.0);
    ...
    fn render_chain_card(
        ...
        i: usize,
        chain: &CausalChain,
        _action: &mut Option<CausalViewAction>,
    ) {
            ...
            .show(ui, |ui| {
                let response = ui.horizontal(|ui| {
                    ...
                    ui.vertical(|ui| {
                        ...
                        ui.horizontal(|ui| {
                            ui.label(
                                RichText::new(format!("{} hops", chain.nodes.len() - 1))
                                    .size(10.0)
                                    .color(TEXT_MUTED),
                            );
                            ...
                            if self.show_confidence {
                                ui.add_space(8.0);
                                let conf_color = if chain.confidence > 0.8 {
                                    SUCCESS
                                } else if chain.confidence > 0.5 {
                                    ACCENT
                                } else {
                                    WARNING
                                };
                                ui.label(
                                    RichText::new(format!("{:.0}% conf", chain.confidence * 100.0))
                                        .size(10.0)
                                        .color(conf_color),
                                );
                            }
    ...
    fn render_chain_list(&mut self, ui: &mut egui::Ui, action: &mut Option<CausalViewAction>) {
        ui.label(
            RichText::new(format!("Found {} causal chains", self.causal_chains.len()))
                .size(13.0)
                .color(TEXT_PRIMARY)
                .strong(),
        );
        ...
        ScrollArea::vertical().max_height(300.0).show(ui, |ui| {
            for i in 0..self.causal_chains.len() {
                if let Some(chain) = self.causal_chains.get(i).cloned() {
                    self.render_chain_card(ui, i, &chain, action);
                    ui.add_space(6.0);
                }
            }
        });
    ...
    fn update(
        ...
        ScrollArea::vertical().max_height(300.0).show(ui, |ui| {
            ...
            if let Some(nodes) = self.causal_chains.first().map(|c| c.nodes.clone()) {
                self.render_tree_node(ui, &nodes, 0, _action);
            }
        });
    }

    fn render_node_details(&mut self, ui: &mut egui::Ui, _action: &mut Option<CausalViewAction>) {
        if self.causal_chains.is_empty() {
            return;
        }
    ...
    fn render_tree_node(
        ...
        nodes: &[CausalNode],
        index: usize,
        action: &mut Option<CausalViewAction>,
    ) {
        if index >= nodes.len() {
            return;
        }
        ...
        ui.horizontal(|ui| {
                ...
                .show(ui, |ui| {
                    ui.horizontal(|ui| {
                        ...
                        if self.show_confidence {
                            ui.add_space(8.0);
                            ui.label(
                                RichText::new(format!("{:.0}%", node.confidence * 100.0))
                                    .size(10.0)
                                    .color(TEXT_MUTED),
                            );
                        }
            ...
            if ui
                .add(
                    egui::Label::new(RichText::new("→").size(10.0).color(PRIMARY))
                        .sense(egui::Sense::click()),
                )
                .clicked()
            {
            ...
            .show(ui, |ui| {
                ...
                for chain in &self.causal_chains {
                    for pair in chain.nodes.windows(2) {
                        ...
                        if let (Some(fi), Some(ti)) = (from_idx, to_idx) {
                            let from_angle =
                                (fi as f32) * std::f32::consts::TAU / (all_nodes.len() as f32);
                            let to_angle =
                                (ti as f32) * std::f32::consts::TAU / (all_nodes.len() as f32);
                            ...
                            painter.add(egui::Shape::convex_polygon(
                                vec![to_pos, arrow_pos + perp * 5.0, arrow_pos - perp * 5.0],
                                ACCENT.gamma_multiply(0.5),
                                Stroke::NONE,
                            ));
                        }
                ...
                for (i, node) in all_nodes.iter().enumerate() {
                    ...
                    painter.circle_stroke(
                        pos,
                        node_radius,
                        Stroke::new(2.0, Color32::WHITE.gamma_multiply(0.3)),
                    );
    ...
    fn render_root_causes(&mut self, ui: &mut egui::Ui, _action: &mut Option<CausalViewAction>) {
        egui::Frame::none()
            ...
            .show(ui, |ui| {
                ...
                if self.root_causes.is_empty() {
                    ui.label(
                        RichText::new("No root causes identified")
                            .size(11.0)
                            .color(TEXT_MUTED),
                    );
                } else {
                    for cause in &self.root_causes {
                        ui.horizontal(|ui| {
                            ...
                            if self.show_confidence {
                                ui.label(
                                    RichText::new(format!("{:.0}%", cause.confidence * 100.0))
                                        .size(10.0)
                                        .color(TEXT_MUTED),
                                );
                            }
        ...
        egui::Frame::none()
            ...
            .show(ui, |ui| {
                ...
                if self.impact_analysis.is_empty() {
                    ui.label(
                        RichText::new("Run analysis to see impacts")
                            .size(11.0)
                            .color(TEXT_MUTED),
                    );
                } else {
    ...
    pub fn analyze(
        &mut self,
        storage: &sutra_storage::ConcurrentMemory,
        effect_id: sutra_storage::ConceptId,
        max_hops: usize,
    ) {
        self.is_analyzing = true;
        self.error_message = None;
        ...
        let snapshot = storage.get_snapshot();
        {
            ...
            for hop in 0..max_hops {
                ...
                if let Some((neighbor_id, weight)) = neighbors
                    .into_iter()
            ...
            if !chain_nodes.is_empty() {
                ...
                let avg_confidence = chain_nodes.iter().map(|n| n.confidence).sum::<f32>()
                    / chain_nodes.len() as f32;
    ...
    pub fn analyze_static(
        ...
        effect_id: sutra_storage::ConceptId,
        max_hops: usize,
    ) -> (Vec<CausalChain>, Vec<CausalNode>) {
