...
use crate::theme::{
    ACCENT, BG_DARK, BG_ELEVATED, BG_WIDGET, PRIMARY, SECONDARY, TEXT_MUTED, TEXT_PRIMARY,
    TEXT_SECONDARY,
};
...
const COMMANDS: &[CommandHint] = &[
    CommandHint {
        command: "/learn",
        shortcut: "/l",
        description: "Teach new knowledge",
        needs_arg: true,
    },
    CommandHint {
        command: "/search",
        ...
        Self {
            messages: vec![Message {
                ...
                content: "👋 Welcome! I'm Sutra, your personal knowledge assistant.\n\n\
                    ...
                    • Browse the Knowledge tab to see what I know"
                    .into(),
                timestamp: Local::now(),
            }],
            input: String::new(),
...
impl ChatPanel {
            ...
            egui::Frame::none()
                ...
                .show(ui, |ui| {
                    ui.horizontal(|ui| {
                        ...
                        ui.add_space(8.0);
                        ui.label(
                            RichText::new("Sutra is thinking...")
                                .size(13.0)
            ...
            .show(ui, |ui| {
                ui.horizontal_top(|ui| {
                    ...
                    let text_edit = TextEdit::multiline(&mut self.input)
                        .hint_text(
                            "Type a question, or / for commands... (Shift+Enter for new line)",
                        )
                        .frame(false)
                    ...
                    if resp.has_focus() {
                        ui.input(|i| {
                            ...
                            if i.key_pressed(Key::Tab)
                                && has_suggestions && self.show_autocomplete {
                                    if self.autocomplete_index < 0 {
                                        self.autocomplete_index = 0;
                                    }
    ...
    fn get_command_suggestions(&self) -> Vec<&'static CommandHint> {
        ...
        COMMANDS
            .iter()
            .filter(|cmd| {
                cmd.command.starts_with(&input_lower) || cmd.shortcut.starts_with(&input_lower)
            })
            .collect()
    }
            ...
            .show(ui, |ui| {
                ...
                for (i, cmd) in suggestions.iter().enumerate() {
                    ...
                    if is_selected {
                        ...
                        ui.painter()
                            .rect_filled(indicator_rect, Rounding::same(2.0), PRIMARY);
                    }
        ...
        ui.horizontal(|ui| {
            ...
            let (bg, border_color, avatar_bg, avatar_icon) = match msg.role {
                ...
                MessageRole::Assistant => {
                    (BG_WIDGET, BG_WIDGET.gamma_multiply(1.2), SECONDARY, "🧠")
                }
                MessageRole::System => (
    ...
    fn parse_command(&mut self, input: &str) -> Option<ChatAction> {
        ...
        if input_lower.starts_with("learn:") {
            let content = input_trimmed.split_once(':').map(|x| x.1).unwrap_or("").trim();
            if content.is_empty() {
    ...
    fn show_help(&mut self) {
        self.messages.push(Message {
            ...
            content: "📚 **Available Commands**\n\n\
                ...
                **Legacy:**\n\
                • `learn: <text>` - Also works for learning"
                .into(),
            timestamp: Local::now(),
        });
    }
