...
use sutra_storage::{ConceptId, ConcurrentMemory};
...
impl GraphView {
    ...
    pub fn load_from_storage(&mut self, storage: &ConcurrentMemory) {
        ...
        for concept in &concepts {
            ...
            let mut node =
                GraphNode::new(concept.id, content, concept.confidence, concept.strength);
    ...
    pub fn ui(&mut self, ui: &mut egui::Ui) -> Option<GraphAction> {
        ...
        // Background
        ui.painter()
            .rect_filled(graph_rect, Rounding::same(12.0), BG_DARK);
        ...
        for (id, node) in &self.nodes {
            let node_screen_pos = self
                .camera
                .world_to_screen(Pos2::new(node.x, node.y), graph_rect);
        ...
        for edge in &self.edges {
            ...
            let from_pos = self
                .camera
                .world_to_screen(Pos2::new(from_node.x, from_node.y), graph_rect);
            let to_pos = self
                .camera
                .world_to_screen(Pos2::new(to_node.x, to_node.y), graph_rect);

            // Skip if outside view
            if !graph_rect.expand(50.0).contains(from_pos)
                && !graph_rect.expand(50.0).contains(to_pos)
            {
                continue;
            }
            ...
            ui.painter()
                .line_segment([from_pos, to_pos], Stroke::new(stroke_width, color));
        }
        ...
        for (id, node) in &self.nodes {
            ...
            ui.painter()
                .circle_stroke(screen_pos, radius, Stroke::new(2.0, base_color));
    ...
    fn render_header(&mut self, ui: &mut egui::Ui, action: &mut Option<GraphAction>) {
        ui.horizontal(|ui| {
            ...
            egui::Frame::none()
                ...
                .show(ui, |ui| {
                    ui.label(
                        RichText::new(format!("{} nodes", self.nodes.len()))
                            .size(12.0)
            ...
            egui::Frame::none()
                ...
                .show(ui, |ui| {
                    ui.label(
                        RichText::new(format!("{} edges", self.edges.len()))
                            .size(12.0)
                            .color(TEXT_SECONDARY),
                    );
                });
    ...
    fn render_legend(&self, ui: &mut egui::Ui, rect: Rect) {
        ...
        for (i, (edge_type, name)) in edge_types.iter().enumerate() {
            ...
            ui.painter().line_segment(
                [
                    Pos2::new(legend_rect.min.x + 10.0, y + 4.0),
                    Pos2::new(legend_rect.min.x + 30.0, y + 4.0),
                ],
    ...
    fn render_controls(&mut self, ui: &mut egui::Ui, _action: &mut Option<GraphAction>) {
        ui.horizontal(|ui| {
            // Center button
            ...
            egui::ComboBox::from_id_salt("layout_type")
                ...
                .show_ui(ui, |ui| {
                    ...
                    if ui
                        .selectable_label(self.layout_type == LayoutType::Radial, "Radial")
                        .clicked()
                    {
                    ...
                    if ui
                        .selectable_label(self.layout_type == LayoutType::Grid, "Grid")
                        .clicked()
                    {
            ...
            // Simulation toggle
            ui.add_space(16.0);
            let sim_text = if self.simulation_running {
                "⏸ Pause"
            } else {
                "▶ Run"
            };
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                if let Some(id) = self.selected {
                    if let Some(node) = self.nodes.get(&id) {
                        ui.label(
                            RichText::new(format!(
                                "Selected: {} | Neighbors: {} | Confidence: {:.0}%",
    ...
    fn render_node_info(&self, _ui: &mut egui::Ui, _node: &GraphNode) {
