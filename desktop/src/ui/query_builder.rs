...
impl QueryBuilder {
    ...
    fn render_header(&mut self, ui: &mut egui::Ui) {
        ui.horizontal(|ui| {
            ...
            ui.add_space(4.0);
            ui.label(RichText::new("Advanced Query Builder").size(22.0).color(TEXT_PRIMARY).strong());
            
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                // Saved queries toggle
                let saved_text = if self.show_saved_queries { "📁 Hide Saved" } else { "📁 Saved Queries" };
    ...
    fn render_query_type_selector(&mut self, ui: &mut egui::Ui) {
        ui.label(RichText::new("Query Type:").size(13.0).color(TEXT_MUTED).strong());
        ui.add_space(8.0);
        ...
        ui.label(RichText::new(description).size(11.0).color(TEXT_MUTED).italics());
    }
    ...
    fn query_type_button(&mut self, ui: &mut egui::Ui, label: &str, query_type: QueryType) {
        ...
        egui::Frame::none()
            ...
            .show(ui, |ui| {
                if ui.add(egui::Label::new(RichText::new(label).size(13.0).color(text_color))
                ...
                match self.query_type {
                    QueryType::TextSearch | QueryType::SemanticSearch => {
                        ...
                        if self.query_type == QueryType::SemanticSearch {
                            ui.add_space(12.0);
                            ui.horizontal(|ui| {
                                ui.label(RichText::new("Top K Results:").size(12.0).color(TEXT_MUTED));
                    ...
                    QueryType::PathFinding => {
                        ui.horizontal(|ui| {
                            ui.vertical(|ui| {
                                ui.label(RichText::new("From Concept:").size(12.0).color(TEXT_MUTED));
                                ui.add_sized(
                                    Vec2::new(200.0, 28.0),
            ...
            .show(ui, |ui| {
                egui::Frame::none()
                    ...
                    .show(ui, |ui| {
                        ...
                        let confidence_text = format!("{:.0}%", self.filters.min_confidence * 100.0);
                        ui.horizontal(|ui| {
                        ...
                        // Max results
                        ui.horizontal(|ui| {
                            ui.label(RichText::new("Max Results:").size(12.0).color(TEXT_SECONDARY));
                            ui.add(egui::Slider::new(&mut self.filters.max_results, 1..=100));
                        });
                        ...
                        ui.label(RichText::new("Relationship Filters:").size(12.0).color(TEXT_MUTED));
                        ui.horizontal(|ui| {
                        ...
                        ui.horizontal(|ui| {
                            ui.label(RichText::new("Min neighbors:").size(12.0).color(TEXT_SECONDARY));
    ...
    fn render_actions(&mut self, ui: &mut egui::Ui, action: &mut Option<QueryBuilderAction>) {
        ui.horizontal(|ui| {
            // Clear button
            if ui.button(RichText::new("Clear All").size(12.0).color(TEXT_SECONDARY)).clicked() {
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                ...
                if ui.add_enabled(
                    can_run,
                    egui::Button::new(RichText::new("▶ Run Query").size(14.0).color(Color32::WHITE))
                        .fill(PRIMARY)
    ...
    fn render_results(&mut self, ui: &mut egui::Ui, action: &mut Option<QueryBuilderAction>) {
        // Results header
        ui.horizontal(|ui| {
            ui.label(RichText::new("Results").size(14.0).color(TEXT_PRIMARY).strong());
            ...
            if !self.results.is_empty() {
                ...
                egui::Frame::none()
                    ...
                    .show(ui, |ui| {
                        ui.label(RichText::new(format!("{} found", self.results.len())).size(11.0).color(SUCCESS));
                    });
                
                if let Some(ms) = self.last_search_time_ms {
                    ui.label(RichText::new(format!("({}ms)", ms)).size(11.0).color(TEXT_MUTED));
                }
            }
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                if !self.results.is_empty() {
                    ...
                    if ui.button(RichText::new("📊 Visualize").size(12.0)).clicked() {
            ...
            egui::Frame::none()
                ...
                .show(ui, |ui| {
                    ui.vertical_centered(|ui| {
                        ...
                        ui.add_space(8.0);
                        ui.label(RichText::new("Enter a query and click Run").size(13.0).color(TEXT_MUTED));
                    });
                });
        } else {
            ScrollArea::vertical()
    ...
    fn render_result_card(&self, ui: &mut egui::Ui, index: usize, result: &ConceptInfo, action: &mut Option<QueryBuilderAction>) {
        egui::Frame::none()
            ...
            .show(ui, |ui| {
                ui.horizontal(|ui| {
                    ...
                    egui::Frame::none()
                        ...
                        .inner_margin(egui::Margin::symmetric(8.0, 4.0))
                        .show(ui, |ui| {
                            ui.label(RichText::new(format!("{}", index + 1)).size(12.0).color(PRIMARY).strong());
                        });
                    ...
                    ui.vertical(|ui| {
                        ...
                        ui.label(RichText::new(&content_preview).size(13.0).color(TEXT_PRIMARY));
                        
                        ui.add_space(4.0);
                        
                        ui.horizontal(|ui| {
                            // Confidence
                            let conf_color = if result.confidence > 0.8 { SUCCESS } else if result.confidence > 0.5 { ACCENT } else { WARNING };
                            ui.label(RichText::new(format!("Confidence: {:.0}%", result.confidence * 100.0))
                                .size(11.0).color(conf_color));
                            
                            ui.add_space(12.0);
                            
                            // ID
                            ui.label(RichText::new(format!("ID: {}...", &result.id[..8]))
                                .size(10.0).color(TEXT_MUTED).monospace());
                            
                            // Neighbors
                            if !result.neighbors.is_empty() {
                                ui.add_space(12.0);
                                ui.label(RichText::new(format!("{} connections", result.neighbors.len()))
        ...
        egui::Frame::none()
            ...
            .show(ui, |ui| {
                if self.saved_queries.is_empty() {
                    ui.label(RichText::new("No saved queries yet").size(12.0).color(TEXT_MUTED));
                } else {
                    for (i, saved) in self.saved_queries.clone().iter().enumerate() {
                        ui.horizontal(|ui| {
                            ...
                            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                                if ui.button(RichText::new("🗑").size(11.0).color(ERROR)).clicked() {
