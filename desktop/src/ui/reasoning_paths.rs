...
use crate::theme::*;
use crate::types::{ConsensusResult, PathCluster, PathStep, ReasoningPath};
use eframe::egui::{self, Color32, RichText, Rounding, ScrollArea, Stroke, Vec2};
use std::collections::HashMap;
use sutra_storage::{ConceptId, ConcurrentMemory, ParallelPathFinder, PathResult};
...
impl ReasoningPathsPanel {
    ...
    fn convert_path(
        &self,
        path: &PathResult,
        snapshot: &std::sync::Arc<sutra_storage::GraphSnapshot>,
    ) -> ReasoningPath {
        let steps: Vec<PathStep> = path
            ...
            .map(|(i, id)| {
                let content = snapshot
                    ...
                    .unwrap_or_else(|| format!("Concept {}", &id.to_hex()[..8]));
        ...
        ui.vertical(|ui| {
            ...
            if self.paths.is_empty() && self.error_message.is_none() {
                ui.vertical_centered(|ui| {
                    ...
                    ui.add_space(16.0);
                    ui.label(
                        RichText::new("Discover Connections")
                            .size(18.0)
                            .color(TEXT_PRIMARY)
                            .strong(),
                    );
                    ui.add_space(8.0);
                    ui.label(
                        RichText::new("Find multi-hop reasoning paths between any two concepts.")
                            .size(14.0)
                            .color(TEXT_MUTED),
                    );
                    ui.add_space(24.0);
                    ...
                    egui::Frame::none()
                        ...
                        .show(ui, |ui| {
                            ui.label(
                                RichText::new("Try searching for:")
                                    .size(12.0)
            ...
            ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    ...
                    let paths_data: Vec<_> = self
                        .paths
                        .iter()
                        .enumerate()
                        .map(|(i, path)| {
                            let is_expanded = self.expanded_path == Some(i);
                            let supports_consensus = self
                                .consensus
                                .as_ref()
                                .map(|c| {
                                    path.path.last().map(|s| s.concept_id)
                                        == Some(c.primary_cluster.destination)
                                })
                                .unwrap_or(false);
                    ...
                    if let Some(consensus) = &self.consensus {
                        if !consensus.alternatives.is_empty() {
                            ui.add_space(16.0);
                            ui.label(
                                RichText::new("Alternative Conclusions:")
                                    .size(13.0)
                                    .color(TEXT_MUTED)
                                    .strong(),
                            );
        ...
        ui.horizontal(|ui| {
            ...
            if !self.paths.is_empty() {
                ...
                egui::Frame::none()
                    ...
                    .show(ui, |ui| {
                        ui.label(
                            RichText::new(format!("{} paths found", self.paths.len()))
                                .size(11.0)
                                .color(TEXT_SECONDARY),
                        );
                    });
    ...
    fn render_query_input(&mut self, ui: &mut egui::Ui, action: &mut Option<ReasoningPathsAction>) {
        egui::Frame::none()
            ...
            .show(ui, |ui| {
                ui.label(
                    RichText::new("Find paths between concepts:")
                        .size(12.0)
                        .color(TEXT_MUTED),
                );
                ui.add_space(8.0);
                ...
                ui.horizontal(|ui| {
                    ...
                    if ui
                        .add_enabled(
                            can_search,
                            egui::Button::new(RichText::new("🔍 Find Paths").size(13.0)),
                        )
                        .clicked()
                    {
        ...
        egui::Frame::none()
            ...
            .show(ui, |ui| {
                ui.horizontal(|ui| {
                    ...
                    ui.add_space(8.0);
                    ui.label(
                        RichText::new(&consensus.primary_cluster.destination_content)
                            .size(14.0)
                            .color(TEXT_PRIMARY)
                            .strong(),
                    );
                    ...
                    egui::Frame::none()
                        ...
                        .show(ui, |ui| {
                            ui.label(
                                RichText::new(format!(
                                    "Confidence: {:.0}%",
                                    consensus.primary_cluster.avg_confidence * 100.0
                                ))
                                .size(11.0)
                                .color(ACCENT),
                            );
                ...
                ui.add_space(4.0);
                ui.label(
                    RichText::new(&consensus.explanation)
                        .size(12.0)
                        .color(TEXT_SECONDARY),
                );
            });
        ...
        egui::Frame::none()
            ...
            .show(ui, |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.label(
                        RichText::new(format!("Path {}", index + 1))
                            .size(13.0)
                            .color(TEXT_PRIMARY)
                            .strong(),
                    );
                    ...
                    egui::Frame::none()
                        ...
                        .show(ui, |ui| {
                            ui.label(
                                ...
                                .size(11.0)
                                .color(self.confidence_color(path.confidence)),
                            );
                        });

                    // Depth
                    ui.label(
                        RichText::new(format!("({} hops)", path.depth))
                            .size(11.0)
                            .color(TEXT_MUTED),
                    );
        ...
        for (i, step) in path.path.iter().enumerate() {
            ui.horizontal(|ui| {
                ...
                ui.vertical(|ui| {
                    ...
                    ui.horizontal(|ui| {
                        ui.label(
                            RichText::new(format!("Confidence: {:.0}%", step.confidence * 100.0))
                                .size(10.0)
                                .color(TEXT_MUTED),
                        );
                        ui.add_space(8.0);
                        ui.label(
                            RichText::new(format!("ID: {}...", &step.concept_id.to_hex()[..8]))
                                .size(10.0)
                                .color(TEXT_MUTED)
                                .monospace(),
                        );
    ...
    fn render_alternative(&self, ui: &mut egui::Ui, alt: &PathCluster) {
        ui.horizontal(|ui| {
            ...
            ui.label(
                RichText::new(&alt.destination_content)
                    .size(12.0)
                    .color(TEXT_SECONDARY),
            );
            ui.label(
    ...
    fn render_controls(&mut self, ui: &mut egui::Ui, action: &mut Option<ReasoningPathsAction>) {
        ui.horizontal(|ui| {
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                if ui
                    .button(RichText::new("📤 Export Reasoning").size(12.0))
                    .clicked()
                {
