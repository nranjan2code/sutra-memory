...
from sutra_core import Concept, Association, AssociationType, ReasoningStep
...
def demo_basic_functionality():
    ...
    print(f"🔗 Created association: {energy_assoc.assoc_type.value} (confidence: {energy_assoc.confidence})")
...
def demo_association_extraction():
    ...
    for text in test_texts:
        ...
        for key, assoc in list(associations.items())[-associations_created:]:
            ...
            print(f"   • {source_concept['content'] if isinstance(source_concept, dict) else source_concept.content}")
            print(f"     --[{assoc.assoc_type.value}]--> ")
            print(f"     {target_concept['content'] if isinstance(target_concept, dict) else target_concept.content}")
